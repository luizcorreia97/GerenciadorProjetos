...
public class RESTRIÇÕES extends JFrame {

	RestriçõesDAO Restrições = new RestriçõesDAO();
	...
	public RESTRIÇÕES(int id_projeto) {
		addWindowListener(new WindowAdapter() {
			@Override
			public void windowActivated(WindowEvent e) {
				Restrições.setId_projeto(id_projeto);
				popular_table();
		...
		btn_cadastrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int valida = JOptionPane.showConfirmDialog(null,"Deseja Realmente Cadastrar ''"
						...
						Restrições.setId_projeto(id_projeto);
						Restrições.setDescricao(txt_descrição.getText());
				
						if(valida == JOptionPane.YES_OPTION){
							Restrições.Cadastrar();
							txt_descrição.setText(null);
							txt_descrição.requestFocus();
							popular_table();
						}
						else {
							JOptionPane.showMessageDialog(null, "''"+Restrições.getDescricao()+"'' Não Foi Cadastrado(a)! =(");
							txt_descrição.setText(null);
							txt_descrição.requestFocus();
						}
			}
		...
		btn_alterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				int valida = JOptionPane.showConfirmDialog(null,"Deseja Realmente Alterar ''"
						...
						if(valida == JOptionPane.YES_OPTION){
							Restrições.setId(Integer.parseInt(table.getValueAt(posicao_table, 0).toString()));
							Restrições.Alterar();
							JOptionPane.showMessageDialog(null, "''"+table.getValueAt(posicao_table, 1).toString()+"'' Alterado(a) Com Sucesso Para ''"+Restrições.getDescricao()+"'' =)");
							txt_descrição.setText(null);
							txt_descrição.requestFocus();
							popular_table();
						}
	...
	public void popular_table(){
		...
		table.setModel(Restrições.Listar());
