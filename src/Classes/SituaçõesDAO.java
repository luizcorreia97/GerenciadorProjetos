...
public class SituaçõesDAO {
	...
	public void Cadastrar() {
		...
		try {
			...
			st.execute("INSERT INTO TB_SITUAÇÕES VALUES ('"+getDescricao()+"')");
			st.close();
	...
	public void Alterar(){
		...
		try{
			...
			Statement st = string_conexao.createStatement();
			st.execute("UPDATE TB_SITUAÇÕES SET DESCRIÇÃO = '"+getDescricao()+"' WHERE ID = "+getId()+"");			
			st.close();
		}
		catch (SQLException e){
	...
	public void Excluir(){
		...
		try{
			Connection string_conexao = new Conecta().getConnection();
			Statement st = string_conexao.createStatement();
			st.execute("DELETE FROM TB_SITUAÇÕES WHERE ID = '"+getId()+"'");
			st.close();
		}
		catch (SQLException e){
	...
	public TableModel Listar(){
		...
		try {
			...
			String sql = "SELECT * FROM TB_SITUAÇÕES";
