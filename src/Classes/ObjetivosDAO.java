...
public class ObjetivosDAO {
	...
	public void Cadastrar() {
		...
		try {
			...
			st.execute("INSERT INTO TB_OBJETIVOS VALUES ("+getId_projeto()+",'"+getDescricao()+"')");
	...
	public void Alterar(){
		...
		try{
			...
			Statement st = string_conexao.createStatement();
			st.execute("UPDATE TB_OBJETIVOS SET DESCRIÇÃO = '"+getDescricao()+"' WHERE ID = "+getId()+" AND ID_PROJETO = "+getId_projeto()+"");			
			st.close();
		}
		catch (SQLException e){
	...
	public void Excluir(){
		...
		try{
			Connection string_conexao = new Conecta().getConnection();
			Statement st = string_conexao.createStatement();
			st.execute("DELETE FROM TB_OBJETIVOS WHERE ID = "+getId()+" AND ID_PROJETO = "+getId_projeto()+"");
	...
	public TableModel Listar(){
		...
		try {
			...
			String sql = "SELECT ID, DESCRIÇÃO FROM TB_OBJETIVOS WHERE ID_PROJETO = "+getId_projeto()+"";
